...
import primaryAccount from '@salesforce/apex/PresciberWorkspaceLWCController.getPrimaryAccount';
...
export default class PrescriberApplicationStepThree extends NavigationMixin(LightningElement) {
    ...
    connectedCallback(){
        ...
        this.fetchPickListValues({'sobjectType' : 'Contact'}, 'State_of_License__c');
        this.primaryAccount();
    }

    primaryAccount(){
        primaryAccount().then(response => {
            if(response != ''){
                //this.accountRecordId = response;
    ...
    get radioOptions(){
        console.log('radioOptions', this.caseData.Product__c);
        ...
        }else if(this.caseData.Product__c == 'ABILIFY ASIMTUFII'){
            return [
                { label: 'Local Care Center', value: 'Local Care Center' },
                { label: 'Prescribing Provider', value: 'Prescribing Provider' },
            ];     
